...
function initLightbox(){
  ...
  if(!lb){
    ...
    lb.innerHTML = '<button class="close" aria-label="close">&times;</button><div class="lb-content"></div>';
    document.body.appendChild(lb);
  }
  const content = lb.querySelector('.lb-content');
  const open = el =>{
    // clone the placeholder (or <img>) so the draft works before real images exist
    content.innerHTML = '';
  ...
  boxes.forEach(b=> b.addEventListener('click', ()=> open(b)));
