...
/* Make.com webhook -> WhatsApp Cloud API (full auto-send). Empty = skip. */
const MAKE_WEBHOOK = 'https://hook.eu1.make.com/tu7fgq7apmd9981nsriqv7ztc4usaxcv';
...
function initContactForm(){
  ...
  form.addEventListener('submit', e=>{
    ...
    // 1) Full auto-send: POST to Make webhook (form-encoded = no CORS preflight)
    if(MAKE_WEBHOOK){
      try{
        fetch(MAKE_WEBHOOK, { method:'POST', body:new URLSearchParams({name, phone, email, detail}) }).catch(()=>{});
      }catch(_){}
    }

    // 2) Fallback: open WhatsApp prefilled for the visitor to tap send
